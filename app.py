...
import os, json, base64, threading, re
...
Example output: [{"hsn":"1234","uqc":"NOS","quantity":10,"taxable_value":5000,"igst":900,"cgst":0,"sgst":0}]"""


def norm(s):
    return str(s or "").lower().strip().replace(r"\s+", " ")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.hsn_map = {}        # consolidated data
        self.result_rows = []    # for editable table
        self._load_config()
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(self.sidebar, text="Save Key", height=30, command=self._save_key).pack(fill="x", padx=16, pady=(0,20))
    ...
    def _build_upload_tab(self):
        ...
        # Drop zone (simulated)
        self.drop_frame = ctk.CTkFrame(tab, height=180, corner_radius=12,
        ...
        ctk.CTkLabel(self.drop_frame, text="Click Browse to select your bills",
                     font=ctk.CTkFont(size=14)).pack()
        ctk.CTkLabel(self.drop_frame, text="JPG · PNG · PDF · XLSX · XLS · CSV",
                     text_color="gray", font=ctk.CTkFont(size=11)).pack(pady=(4,0))
        ctk.CTkButton(self.drop_frame, text="Browse Files", width=160,
                      command=self._browse_files).pack(pady=(12,0))
    ...
    def _build_process_tab(self):
        ...
        ctk.CTkLabel(tab, text="Processing Log", font=ctk.CTkFont(size=13, weight="bold"),
        ...
        self.log_box = ctk.CTkTextbox(tab, font=ctk.CTkFont(family="Courier New", size=11),
                                       fg_color=("#1a1a2e","#1a1a2e"), text_color="#00ff88",
                                       state="disabled")
    ...
    def _build_results_tab(self):
        ...
        ctk.CTkLabel(tab, text="Click any cell to edit  ·  All values editable before download",
    ...
    def _refresh_file_sidebar(self):
        ...
        for f in self.files:
            ...
            ctk.CTkLabel(row, text=ext, width=42, fg_color=c,
                          corner_radius=4, font=ctk.CTkFont(size=9, weight="bold")).pack(side="left")
            name = Path(f).name
        ...
        n = len(self.files)
        self.upload_status.configure(text=f"{n} file{'s' if n!=1 else ''} ready to process.")
    ...
    def _start_processing(self):
        ...
        if not key:
            messagebox.showerror("API Key Missing", "Please enter your Anthropic API key.")
            return
        self.api_key = key
        self._save_config()
        ...
        for idx, fpath in enumerate(self.files):
            ...
            try:
                ...
                if ext in (".jpg", ".jpeg", ".png"):
                    ...
                    self._log(f"  ✗ Unsupported format, skipping.")
                    continue

                self._merge_rows(rows, name)
                self._log(f"  ✓ Done — {len(rows)} line item(s) extracted.")

            except Exception as e:
                self._log(f"  ✗ ERROR: {e}")

            self.after(0, self.progress_bar.set, (idx + 1) / total)

        self._log("\n── All files processed. ─────────────────────")
        self.after(0, self._show_results)

    def _process_image(self, client, fpath):
        self._log("  → Reading image via Vision AI...")
    ...
    def _process_pdf(self, client, fpath):
    ...
    def _call_claude(self, client, b64, media_type, label):
    ...
    def _process_excel(self, fpath):
        self._log("  → Parsing Excel/CSV...")
        ext = Path(fpath).suffix.lower()
        import csv
        ...
        for i, row in enumerate(raw[:10]):
            row_norm = [str(c or "").lower().strip() for c in row]
            if any(a in " ".join(row_norm) for a in HSN_ALIASES + QTY_ALIASES):
                header_idx = i
                break
    ...
    def _refresh_results(self):
        ...
        self.stat_labels["Taxable Value"].configure(
            text=f"₹{tv:,.0f}")
        self.stat_labels["Total Tax"].configure(
            text=f"₹{tax:,.0f}")
        ...
        if "UNKNOWN" in self.hsn_map:
            ...
            self.flag_label.configure(
                text=f"⚠  Some items are missing HSN codes "
                     f"(Qty: {u['quantity']:.2f} | "
                     f"Taxable: ₹{u['taxable_value']:,.2f}). "
    ...
    def _edit_cell(self, event):
        ...
        # Update hsn_map
        field_map = {
            "HSN Code": "hsn", "UQC": "uqc", "Quantity": "quantity",
    ...
    def _add_row(self):
        ...
        self.hsn_map[key] = {"hsn": key, "uqc": "NOS",
                              "quantity": 0, "taxable_value": 0,
                              "igst": 0, "cgst": 0, "sgst": 0}
        self._refresh_results()
    ...
    def _open_flag_dialog(self):
        ...
        info = (f"Quantity: {u['quantity']:.2f}  |  "
                f"Taxable: ₹{u['taxable_value']:,.2f}  |  "
                f"IGST: ₹{u['igst']:.2f}  |  "
                f"CGST: ₹{u['cgst']:.2f}  |  "
                f"SGST: ₹{u['sgst']:.2f}")
        ctk.CTkLabel(win, text=info, wraplength=380, text_color="gray",
                     font=ctk.CTkFont(size=11)).pack(padx=20, anchor="w")
        ...
        def apply():
            ...
            if not hsn:
                ...
                return
            if hsn in self.hsn_map:
                self.hsn_map[hsn]["quantity"]      += u["quantity"]
                self.hsn_map[hsn]["taxable_value"] += u["taxable_value"]
                self.hsn_map[hsn]["igst"]          += u["igst"]
                self.hsn_map[hsn]["cgst"]          += u["cgst"]
                self.hsn_map[hsn]["sgst"]          += u["sgst"]
            else:
    ...
    def _download_excel(self):
        ...
        headers = ["HSN/SAC Code", "UQC", "Total Quantity",
                   "Total Taxable Value", "Integrated Tax (IGST)",
                   "Central Tax (CGST)", "State/UT Tax (SGST)"]
        ...
        for ri, e in enumerate(entries, 2):
            ...
            row = [e["hsn"], e["uqc"],
                   round(e["quantity"], 2), round(e["taxable_value"], 2),
        ...
        ws.cell(row=tr, column=2).font = Font(bold=True, name="Arial", size=10)
        ws.cell(row=tr, column=2).fill = PatternFill("solid", start_color="D6E4F0")
